...
function showTab(n : number) : void {
    ...
    var inputLabel = "";
    if (currentTab == 1) {
        inputLabel = "Can I Know your gender.";
    ...
    if (n == x.length - 1) {
        (document.getElementById("nextBtn") as HTMLElement).innerText = "Submit";
    } else {
        (document.getElementById("nextBtn") as HTMLElement).innerText = "Next";
    }
}
...
function nextPrev(n : number): void | boolean {
    ...
    if (currentTab >= x.length) {
        ...
        (document.getElementById("regID") as HTMLElement).innerText = "Your resgisteration id is " + xy;
